...
- Refresh-governed artifacts — `AGENTS.md`, playbooks, skills, wrappers, shims, hooks — are toolkit-owned: no agent or out-of-band edit is legitimate; route changes to the owner, and refresh restores divergence to the shipped set. Do not lint or format installed copies. Files refresh only seeds (`.agents/push-policy.md`) are repo-owned and editable; durable repo-specific rules go to `.agents/repo-guidance.md`, facts to the other `.agents/` files.
